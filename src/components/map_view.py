...
def render_map_view(city_data, segments, sensors_df, reports_df):
    ...
    for seg in segments:
        ...
        <div style="font-family: 'Plus Jakarta Sans', Arial, sans-serif; width: 230px; padding: 6px; color: #1E293B;">
            ...
            <div style="font-size: 11px; margin-bottom: 8px; color: #64748B;">ID: {seg['segment_id']}</div>
    ...
    for _, row in sensors_df.iterrows():
        ...
        <div style="font-family: Arial, sans-serif; width: 190px; padding: 4px; color: #0F172A;">
            ...
            <b>Telemetry:</b> Real-time streaming
        </div>
        """
        folium.CircleMarker(
    ...
    for _, row in reports_df.iterrows():
        ...
        folium.Marker(
            ...
            tooltip=f"Citizen Alert: {row['category']} ({row['severity']})",
    ...
    # Visual Map Legend
    st.markdown("""
    <div style="display: flex; gap: 20px; flex-wrap: wrap; background: rgba(18, 23, 33, 0.7); padding: 12px 16px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.06); font-size: 0.85rem; margin-top: 8px;">
        ...
        <div style="display: flex; align-items: center; gap: 6px;">
            <span style="width: 10px; height: 10px; border-radius: 50%; background: #00F2FE;"></span> IoT Water Sensor Node
        </div>
        <div style="display: flex; align-items: center; gap: 6px;">
            <span style="color: #F87171; font-weight: bold;">📍</span> Citizen Science Alert
        </div>
    </div>
